...
export default function SearchScreen() {
  ...
  // Fetches the seach from free dictionary api
  const handleFetch = () => {
    // Checks that user wrote something
    if (!search.trim()) {
    ...
    // Fetches definitions using the dictionary API
    fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${search}`)
      ...
      .then(data => {
        if (data.title === "No Definitions Found") {
          ...
          setResult(definitions); // sets definitions 
          setErrorMessage('');
        }
        setSearch(''); // clears the search
      })
      .catch(err => {
        console.error(err);
  ...
  return (
    <View style={[styles.container, { flex: 1, justifyContent: 'flex-start' }]}>
      <Text style={styles.text}>Enter a word to get its definitions</Text>
      <TextInput
        style={styles.input}
